...
function renderThreadList() {
    ...
    Object.keys(threads).reverse().forEach(id => {
        ...
        title.className = "thread-title"; // Thêm class để dễ style CSS
        title.style.flex = "1";
        title.style.overflow = "hidden";
        title.style.textOverflow = "ellipsis";
        title.style.whiteSpace = "nowrap";
        title.innerText = threads[id].title;
        title.onclick = () => loadThread(id);
...
function deleteThread(id) {
    if (confirm("Xóa cuộc hội thoại này?")) {
        let users = JSON.parse(localStorage.getItem("chat_users"));
        delete users[currentUser].threads[id];
        localStorage.setItem("chat_users", JSON.stringify(users));
        
        if (id === currentThreadId) {
            const remainingIds = Object.keys(users[currentUser].threads);
            if (remainingIds.length > 0) {
                loadThread(remainingIds[remainingIds.length - 1]);
            } else {
                createNewThread();
            }
        } else {
...
async function sendMessageAction() {
    ...
    if (thread.title === "Chủ đề mới") {
        thread.title = val.substring(0, 20) + "...";
    }
    ...
    try {
        const res = await fetch("/api/chat", {
            ...
            body: JSON.stringify({
                messages: [
                    { 
                        role: "system", 
                        content: "Bạn là Giáo sư AI thông thái. Trả lời bằng tiếng Việt. Sử dụng Markdown để trình bày (danh sách, bảng, in đậm). Nếu có công thức toán học, LUÔN dùng ký hiệu $...$ cho công thức nội dòng và $$...$$ cho công thức xuống dòng." 
                    }, 
                    ...thread.messages.slice(-10) // Tăng context lên 10 tin nhắn
                ]
            })
        });
        ...
        if (data.choices && data.choices[0] && data.choices[0].message) {
            ...
            // Render lại toán học ngay sau khi tin nhắn bot hiện ra
            if (window.MathJax) MathJax.typesetPromise();
        } else {
